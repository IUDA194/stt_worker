...
async def test_asr_client_transcribe_ok():
    ...
    out = await client.transcribe(b"BYTES", filename="a.ogg", language="ru", task="transcribe")
    assert out == "hello world"
    assert route.called


@respx.mock
async def test_asr_client_bad_payload():
    client = ASRClient("http://asr.example.com")
...
async def test_asr_client_plain_text_response():
    ...
    out = await client.transcribe(b"BYTES")
